...
interface AtlasDirectiveProps {
  ...
  className?: string;
  narrativeChunk?: string; // Optional: specify which chunk to load, defaults to main chunk
  enableMultiChunk?: boolean; // Optional: enable loading multiple chunks
}

/**
 * Enhanced ATLAS Directive Component with Chunk3 Borisov Chemistry Support
 *
 * Features:
 * - Dynamic narrative chunk loading from /data folder
 * - Enhanced error handling for missing or corrupted chunks
 * - Borisov Chemistry branch integration
 * - Improved token economy and achievement display
 * - Chunk validation and fallback mechanisms
 */
export const AtlasDirective: React.FC<AtlasDirectiveProps> = ({
  ...
  /**
   * Enhanced narrative tree loading with multi-chunk support and validation
   */
  useEffect(() => {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            throw new Error(`Narrative chunk not found: ${chunkPath}. Please ensure the chunk file exists in the /data folder.`);
          }
        ...
        if (data.meta && data.nodes.length !== data.meta.total_nodes) {
          console.warn(`Node count mismatch: meta claims ${data.meta.total_nodes} but found ${data.nodes.length} nodes`);
        }

        // Validate root_id exists in nodes
        if (!data.nodes.find(n => n.id === data.root_id)) {
          throw new Error(`Root node "${data.root_id}" not found in chunk nodes`);
        }
        ...
        console.log(`Successfully loaded chunk: ${data.meta?.title || 'Unknown'} with ${data.nodes.length} nodes`);
      } catch (err) {
        ...
        if (enableMultiChunk && narrativeChunk !== 'narrative_tree_complete_12_nodes.json') {
          console.log('Attempting fallback to default chunk...');
          // Note: In a real implementation, you might want to retry with a default chunk
        }
      }
    };
  ...
  const applyBehavioralTraits = useCallback((state: GameState, choice: Choice): GameState => {
    ...
    // Analytical choices (including chemistry-specific)
    if (choiceText.includes('analyze') || choiceText.includes('study') || choiceText.includes('research') ||
        choiceId.includes('scientific') || choiceId.includes('chemical') || choiceId.includes('spectroscopic')) {
  ...
  // Enhanced error state rendering with chunk-specific information
  ...
  // Enhanced loading state with chunk information
